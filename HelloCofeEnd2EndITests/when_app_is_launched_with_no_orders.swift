...
final class when_deleting_an_order: XCTestCase{
    ...
    func test_should_delete_order_sucessfully(){
        
        let collectioViewQuery = XCUIApplication().collectionViews
        let cellsQuery = collectioViewQuery.cells
        let element = cellsQuery.children(matching: .other).element(boundBy: 1).children(matching: .other).element
        element.swipeLeft()
        collectioViewQuery.buttons["Delete"].tap()
